...
- “本周文献精读”必须另起一页；其标题与文献精读表格必须连续排版。
- 数字区间统一使用 `～`，例如 `3～5 μm`；不得使用连字符或短横线表示区间。
...
- 交付前必须从 DOI 指向的出版社/期刊官方页面核验通讯作者身份和所属单位；Crossref 只能用于候选筛选。
- 无法获得“通讯作者 + 单位 + 官方来源链接”完整证据链的候选必须跳过；所有候选均无法核验时停止且不生成 DOCX。
...
- “通讯作者单位”只写通讯作者姓名和所属单位；不能可靠确认时，重新选文献，不得生成带占位说明的 Word。官方链接仅保存在脚本文献元数据中用于核验。
