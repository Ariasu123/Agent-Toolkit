...
class WorkspaceGuard:
    ...
    def resolve(self, requested: str, operation: str = "access") -> Path:
        ...
        if not _is_relative_to(lexical, self.workspace):
            raise WorkspaceAccessError(
                f"{operation} denied: path is outside workspace {self.workspace}: {requested}"
            )
        try:
        ...
        write_operation = operation.lower() in {"write", "edit", "delete", "create"}
        protected = self._is_protected(requested_rel, self.protect_paths) or self._is_protected(
            resolved_rel, self.protect_paths
        )
    ...
    def open_file(
        ...
        *,
        ...
        if (
            ...
            raise WorkspaceAccessError(
                f"{operation} denied: this platform cannot enforce race-safe workspace file access"
            )
        ...
        try:
            ...
            safe_flags = flags | no_follow | close_on_exec | getattr(os, "O_NONBLOCK", 0)
            file_fd = os.open(relative.parts[-1], safe_flags, mode, dir_fd=current_fd)
    ...
    def protected_existing_paths(self) -> list[Path]:
        ...
        for pattern in self.protect_paths:
            iterator = self.workspace.glob(pattern) if "/" in pattern else self.workspace.rglob(pattern)
