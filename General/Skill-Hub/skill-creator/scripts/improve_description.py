...
uses the session's AI tool auth, no separate ANTHROPIC_API_KEY needed).
"""
...
def _call_claude(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    # AI session. The guard is for interactive terminal conflicts;
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for an AI skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that the AI sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
